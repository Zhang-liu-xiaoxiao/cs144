...
class NetworkInterface {
  private:
    ...
    std::unordered_map<uint32_t,std::pair<size_t ,EthernetAddress>> _ip_eth_table{};

    std::vector<std::pair<uint32_t ,InternetDatagram>> _wait_ip_data{};

    std::unordered_map<uint32_t,size_t> _arq_req{};
  ...
  public:
    ...
    void build_push_IP_data(const InternetDatagram &dgram, uint32_t next_hop_ip);
};
